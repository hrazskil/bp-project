...
constStruct = utilities.constants.giveConstants();
...
indexMatrix = ...
    sphericalVectorWaves.indexMatrix(maxL);

indexMatrix = sortrows(indexMatrix.', 5).';

indexMatrix = indexMatrix(:, indexVec);
k0 = 2*pi*frequency/constStruct.c0;
constant = fVec.' .* 1/k0 .* ...
...
FTheta = k0*sqrt(Z0)*FTheta;
FPhi = k0*sqrt(Z0)*FPhi;
