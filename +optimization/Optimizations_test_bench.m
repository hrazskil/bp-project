...
load('C:\Users\kilia\Plocha\gitHub\bp-project\graphical test\test_structure_1\halfwaveDipoleFarFields.mat');
...
Ntheta = 50;  % Number of points for theta (0 to pi)
Nphi = 50;    % Number of points for phi (0 to 2pi)

% Define the angle values for theta and phi
theta_vals = linspace(0, pi, Ntheta);  % Theta values from 0 to pi
...
[theta_grid2, phi_grid2] = meshgrid(pi/2*ones(1, Nphi), phi_vals);  % For Plane 2 (phi varies, theta = pi/2)

% Convert spherical to Cartesian coordinates for Plane 1 (theta varies, phi = 0)
r = 1;  % Fixed distance for simplicity
[x1, y1, z1] = utilities.transforms.sph2cartCoor(r, theta_grid1, phi_grid1);  % Plane 1 points

% Convert spherical to Cartesian coordinates for Plane 2 (phi varies, theta = pi/2)
[x2, y2, z2] = utilities.transforms.sph2cartCoor(r, theta_grid2, phi_grid2);  % Plane 2 points

% Now, x1, y1, z1 contain the cartesian coordinates for Plane 1
% and x2, y2, z2 contain the cartesian coordinates for Plane 2

% Optionally, you can combine these points into a single matrix
plane1_points = [x1(:), y1(:), z1(:)];  % Flattening to get a list of points
plane2_points = [x2(:), y2(:), z2(:)];  % Flattening to get a list of points


% Compute the far-field at the observation points
farField_plane1 = fieldEvaluation.farField(plane1_points, dip, f0List);  % Example function call
farField_plane2 = fieldEvaluation.farField(plane2_points, dip, f0List);  % Example function call



% %% --- 1_d Compute Far-Field Parameters lebedev ---
% % Define physical constants
% construct = utilities.constants.giveConstants();
% omega = 2 * pi * f0List;  % Angular frequency
% k = omega / construct.c0;    % Wavenumber
...
% Nleb = 50;                 % Number of Lebedev quadrature points
% 
...
optimFun = @(amp) ...
    ...
    1i * amp((numDipoles+1):(numDipoles*2)).', dip, f0List, points, ...
...
%     optimization.normObjectiveFunction(amp(1:numDipoles).' + ...
%     1i * amp(numDipoles+1:end).', dip, f0List, points, weights, ...
%     fF_ref, totalPower_ref);
% 
...
% 
% fF_Fmincon = fieldEvaluation.farField(rObserved, dipoleFmincon, f0List);
...
% 
% fF_ga = fieldEvaluation.farField(rObserved, dipoleGa, f0List);
