function [fSW] = projectEsTof(lmax, k0, EsCart, gridEs, weights)
...
%   weights: Lebedev's weights, double [Nleb x 1]
%
%  OUTPUTS
...
[r, theta, phi] = utilities.transforms.cart2sphCoor(...
    gridEs(:,1), gridEs(:,2), gridEs(:,3));

Np = size(gridEs,1);

% transform E to spherical coordinates
Esph = nan(size(EsCart));
[Esph(:,1), Esph(:,2), Esph(:,3)] = utilities.transforms.cart2sphVec(...
    gridEs(:,1), gridEs(:,2), gridEs(:,3), ...
    EsCart(:,1), EsCart(:,2), EsCart(:,3));
...
for ip = 1:Np
    ...
    fSW(indexMatrix(5, ind),1) = fSW(indexMatrix(5, ind),1) + weights(ip,1)*tmp11(ind,1);
    
    % sigma = 2, tau = 1
    ind = indexMatrix(4, :) == 1 & indexMatrix(3, :) == 2;
    fSW(indexMatrix(5, ind),1) = fSW(indexMatrix(5, ind),1) + weights(ip,1)*tmp21(ind,1);
    
    % sigma = 1, tau = 2
    ind = indexMatrix(4, :) == 2 & indexMatrix(3, :) == 1;
    fSW(indexMatrix(5, ind),1) = fSW(indexMatrix(5, ind),1) + weights(ip,1)*tmp12(ind,1);
    
    % sigma = 2, tau = 2
    ind = indexMatrix(4, :) == 2 & indexMatrix(3, :) == 2;
    fSW(indexMatrix(5, ind),1) = fSW(indexMatrix(5, ind),1) + weights(ip,1)*tmp22(ind,1);
